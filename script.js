...
function useBuiltIn(){
  ...
  populateSections();
  setStatus(`Loaded ${Object.keys(data).length} sections from the supplied September 2026 workbooks.`,"success");
}
...
function loadExcel(){
  ...
  reader.onload=e=>{
    try{
      ...
      data=parsed;saveSharedAttendanceData(data);populateSections();
...
function downloadPDF(type){
  ...
  const reports=type==="complete"
    ? [{students:result.present,status:"Present"},{students:result.absent,status:"Absent"}]
    : [{students:result[type],status:type==="present"?"Present":"Absent"}];
  ...
  printableReports.forEach((report,index)=>{
    ...
    pdf.setFontSize(14);pdf.text(`${report.status === "Absent" ? "Absent Students List" : `${report.status} Students`} Class ${selected}`,14,34);pdf.text(today,196,34,{align:"right"});
    pdf.autoTable({startY:39,head:[["S.No","I.D","Name","F/Name",report.status]],body:report.students.map((student,index)=>[index+1,student.id,student.name,student.father,report.status]),theme:"grid",styles:{font:"helvetica",fontSize:9,textColor:[0,0,0],lineColor:[0,0,0],lineWidth:0.2,cellPadding:2.2},headStyles:{fillColor:[255,192,0],textColor:[0,0,0],fontStyle:"bold",halign:"center"},columnStyles:{0:{halign:"center",cellWidth:13},1:{halign:"center",cellWidth:18},2:{cellWidth:57},3:{cellWidth:57},4:{halign:"center",cellWidth:25}}});
  });
...
function resetAll(){data={};selected="";result={present:[],absent:[],unknown:[]};clearSharedAttendanceData();document.getElementById("sectionSelect").innerHTML="";clearEntry();document.getElementById("fileStatus").innerHTML="";document.getElementById("total").textContent=0;document.getElementById("present").textContent=0;document.getElementById("absent").textContent=0;document.getElementById("unknown").textContent=0;renderTable("presentTable",[],"Present");renderTable("absentTable",[],"Absent");}
function resetAll(){data={};selected="";result={present:[],absent:[],unknown:[]};clearSharedAttendanceData();clearDailyAttendance();document.getElementById("sectionSelect").innerHTML="";clearEntry();document.getElementById("fileStatus").innerHTML="";document.getElementById("total").textContent=0;document.getElementById("present").textContent=0;document.getElementById("absent").textContent=0;document.getElementById("unknown").textContent=0;renderTable("presentTable",[],"Present");renderTable("absentTable",[] ,"Absent");}
function setToday(){
